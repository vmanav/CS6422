...
def draw_route(canvas, stop_coords):
    ...
    for i in range(len(stop_coords) - 1):
        x1, y1 = stop_coords[i]
        x2, y2 = stop_coords[i + 1]
        canvas.create_line(x1, y1, x2, y2, fill="black", width=2)

def draw_bus(canvas, stop_coords, current_stop, color="red"):
    x, y = stop_coords[current_stop]
    canvas.create_rectangle(x-10, y-10, x+10, y+10, fill=color, outline="black")
